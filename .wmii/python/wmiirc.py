...
# Keys
keys.defs = dict(
    mod='Mod1',# Mod5 (AltGr) default, now Mod1 (Alt)
    mod2='Mod5',# For monitor switching
    left='h',
    down='j',
    up='k',
    right='l')
...
wmii['font'] = '-*-fixed-medium-r-*-*-13-*-*-*-*-*-*-*'
wmii['normcolors'] = '#cccccc', '#666666', '#333333'
wmii['focuscolors'] = '#ffffff', '#285577', '#4c7899'
wmii['grabmod'] = keys.defs['mod']
wmii['border'] = 2
"""
background = '#222222'
floatbackground='#111111'

wmii['font'] = '-*-fixed-medium-r-*-*-12-*-*-*-*-*-*-*'
wmii['normcolors'] = '#999999', '#222222', '#000000'
wmii['focuscolors'] = '#cccccc', '#333333', '#555555'
wmii['grabmod'] = keys.defs['mod']
wmii['border'] = 2
...
def bind_num(i):
    ...
    keys.bind('main', (
        "Switch monitor",
        ('%%(mod2)s-%d' % i, "Switch to monitor '%d'" % (i - 1),
			lambda k: call('sh', '-c', "DISPLAY=:0.%d swarp $(xdotool getmouselocation 2>/dev/null | sed 's/x:\([0-9]\+\) y:\([0-9]\+\) .*/\\1 \\2/')" % (i - 1))),
			#lambda k: call("sh", "-c", "xdotool mousemove --screen %d $(xdotool getmouselocation 2>/dev/null | sed 's/x:\([0-9]\+\) y:\([0-9]\+\) .*/\\1 \\2/')" % (i - 1))),
    ))
